...
export function AssociateGroupModal({ isOpen, onClose, subject }: AssociateGroupModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Por enquanto, vamos atualizar com apenas um grupo (o primeiro selecionado)
      // No futuro, se a API suportar múltiplos grupos, podemos expandir
      const groupId = selectedGroups.length > 0 ? selectedGroups[0] : undefined;
      ...
      await updateSubjectMutation.mutateAsync({ subjectId: subject.id, payload });
